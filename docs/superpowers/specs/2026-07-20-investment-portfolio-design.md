...
## 4. Business Logic & Service Abstractions (`lib/investments/`)

The business logic is split into dedicated service files inside the `lib/investments/` directory to make it clean, modular, and reusable:

```
lib/investments/
  ├── transactions.ts       # Handles transaction execution & validation rules
  ├── holdings.ts           # Recalculates aggregated position states
  ├── prices.ts             # Market data service abstraction
  ├── calculations.ts       # Pure calculations and backtracking rules
  └── portfolio-viewmodel.ts # ViewModel mapping layer for UI presentation
```

### 4.1 Market Data Service Abstraction (`lib/investments/prices.ts`)
To prevent coupling the application directly to the database collection, market pricing is abstracted behind a clean interface:
* `getLatestPrice(holdingId: string): Promise<number>`
* `getHistoricalPrice(holdingId: string, date: Date): Promise<number>`
* `getHistoricalPrices(holdingId: string, startDate: Date, endDate: Date): Promise<{price: number, date: Date}[]>`

Initially, this service queries the `investment_prices` collection. In the future, this abstraction will allow plugging in live financial APIs (Yahoo Finance, Alpaca, or Polygon) without altering any calculations or UI code.

### 4.2 ViewModel Layer (`lib/investments/portfolio-viewmodel.ts`)
Pages consume a unified ViewModel rather than call calculations directly. The ViewModel maps calculated results into formatted, presentation-ready models.
* **DashboardViewModel**: Holds metrics, lists of brokerage account summaries, overall allocation series, performance timeline charts, watchlists, and activity records.
* **AccountDetailsViewModel**: Specific brokerage account details including holdings lists, transaction ledger, and cash movements.
* **HoldingDetailsViewModel**: Information for a single holding (price trends, purchase timeline, realized/unrealized return).

### 4.3 Transaction Handlers (Strategy Pattern)
Each trade type is processed by its own strategy/handler inside `transactions.ts`:
...
### 4.4 Atomicity
All write operations execute inside a MongoDB transaction session. If any write fails (Wallet balance increment, Holding updates, or Transaction logs), the entire operation rolls back.

---

## 5. Calculations Engine & Historical Backtracking

### 5.1 Dynamic Holdings Calculation
...
3. **Historical Security Price (`calculateHoldingPriceAt`):**
   * Lookup the latest price using the market data service abstraction where $\text{date} \le D$.
...
/investments/accounts                  # Brokerage Accounts Tab / Details List
